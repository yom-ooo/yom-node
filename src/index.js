...
const instructionHandlers = {
  ...
  async bootGame(instruction, status, downloadFolder, numOfThreads) {
    const { gameImageName } = instruction;
    ...
    const configJson = {
      YomCore: {
        ...
        ServerIP: process.env.SPACE_URL, // Confirm if this is the correct environment variable
      },
    };
...
(async () => {
  ...
  const downloadFolder = path.resolve(process.env.STORAGE_LOCATION);
  const walletId = process.env.WALLET_KEY;
  ...
  try {
    ...
    console.log(`CPU: ${hardwareInfo.cpuName || 'N/A'}`);
    console.log(`GPU: ${(hardwareInfo.gpuNames && hardwareInfo.gpuNames[0]) || 'N/A'}`);
    ...
    Object.assign(hardwareInfo, cachedHwInfo);
    ...
    setInterval(async () => {
      try {
        ...
        for (const instruction of instructions) {
          ...
          if (handler) {
            await handler(instruction, status, downloadFolder, status.running.length);
          } else {
